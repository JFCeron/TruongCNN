...
def num_positives_negatives (data_subfolder="Pat3Train", W=30, O=(0,5.2/6)):
    ...
    W_ticks = np.floor(W*constants.F)
    O_ticks = (np.floor(O[0]*W*constants.F), np.floor(O[1]*W*constants.F))
...
def signal2image (multi_signal, w=1, o=2/3):
    ...
    for i in range(len(multi_signal.columns)):
        ...
        values = stft(multi_signal.iloc[:,i], fs=constants.F, nperseg=nperseg, noverlap=noverlap, boundary=None)[2]
        if i == 0:
...
def generate_images (data_subfolder="Pat3Train", W=30, O=(0,5.2/6), w=1, o=2/3, maxPerClass=40000, channels=[i for i in range(16)]):
    ...
    channels = constants.ch_names[channels]
    ...
    for csv_file in set(intervals.file):
        ...
        segment = pd.read_csv("dataset/"+csv_file) # hay que arreglar el valor de dropout
